...
def MST_PrimJarnik(g, start):
  ...
  for v in g.keys():
    ...
    #pqlocator[v] = (d[v], (v, None))
    pq[v] = (d[v], (v, None))

  # for key, value in pqlocator.items():
  #     print('pqlocator', key, value)
  while pq:
    ...
    key = lowest_pq(pq)
    # print('key', key)
    u, edge = pq[key][1]  # unpack tuple from pq
    del pq[key]
...
def MST_Kruskal(g):
  ...
  return tree
def lowest_pq(pq: dict):
  lowest_cost = float('inf')
  lowest_node = None

  for key, value in pq.items():
    if lowest_cost >= value[0]:
      lowest_cost = value[0]
      lowest_node = key
